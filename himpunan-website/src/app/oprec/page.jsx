...
import OprecPDFDocument from '@/components/OprecPDFDocument';

// PDFDownloadLink harus di-load client-only (react-pdf tidak support SSR)
const PDFDownloadLink = dynamic(
  () => import('@react-pdf/renderer').then((mod) => mod.PDFDownloadLink),
  { ssr: false, loading: () => <PdfButtonSkeleton /> }
);
...
export default function OprecPage() {
  ...
  useEffect(() => {
    if (submittedApplicant && qrCanvasRef.current) {
      // Beri sedikit delay agar canvas benar-benar sudah ter-render
    ...
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-lg flex-col items-center justify-center px-4 text-center">
        ...
        <p className="mt-3 text-sm leading-relaxed text-slate-500 sm:text-base">
          Mohon maaf, Open Recruitment Himpunan Mahasiswa saat ini belum dibuka. Pantau terus
          media sosial dan website kami untuk informasi pembukaan pendaftaran selanjutnya.
        </p>
      </div>
    );
  }
      ...
      <section className="bg-[#0F172A] py-16 lg:py-20">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm text-slate-300 sm:text-base">
            Isi form di bawah ini dengan lengkap dan jujur. Tidak perlu login — cukup satu kali
            isi dan submit.
          </p>
        </div>
      </section>
